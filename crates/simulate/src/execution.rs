...
mod tests {
    ...
    use starknet_rust::core::types::Felt as StarknetFelt;
    use std::cell::RefCell;

    /// Builds a `ContractCall` carrying `result`, going through
    /// `from_cheatnet_state_calltrace` so the cheatnet -> ContractCall field mapping
    /// is exercised too.
    fn contract_call_with_result(result: TraceDataCallResult) -> ContractCall {
        let call_trace = RefCell::new(CallTrace {
            ...
            used_syscalls_vm_resources: Default::default(),
            used_syscalls_sierra_gas: Default::default(),
            vm_trace: None,
            vm_memory: None,
            gas_consumed: 0,
            events: vec![],
        ...
        let borrowed = call_trace.borrow();
        ContractCall::from_cheatnet_state_calltrace(&borrowed, 1, 0, 0, false)
    }

    fn map_with(call: ContractCall) -> HashMap<u32, ContractCall> {
        HashMap::from([(1u32, call)])
    }

    #[test]
    fn successful_deepest_call_yields_succeeded() {
        let calls = map_with(contract_call_with_result(Ok(CallSuccess {
            ret_data: vec![],
        })));

        let result = get_execution_result(&calls, Some(1), None, None).unwrap();

        assert!(matches!(result, ExecutionResult::Succeeded));
    }

    #[test]
    fn recoverable_failure_reverts_with_decoded_panic_data() {
        let panic_data = vec![StarknetFelt::from_bytes_be_slice(b"Insufficient balance")];
        let calls = map_with(contract_call_with_result(Err(
            TraceDataCallFailure::Recoverable {
                panic_data: panic_data.clone(),
            },
        )));
        ...
        match result {
            ...
            ExecutionResult::Succeeded => panic!("expected a revert, got Succeeded"),
        }
    }

    #[test]
    fn unrecoverable_failure_reverts_with_error_message() {
        let calls = map_with(contract_call_with_result(Err(
            TraceDataCallFailure::Unrecoverable {
                msg: ByteArray::from("  Entry point not found  "),
            },
        )));
        ...
        match result {
            ExecutionResult::Reverted { reason } => {
                assert_eq!(reason, "Entry point not found");
            }
            ExecutionResult::Succeeded => panic!("expected a revert, got Succeeded"),
        }
    }
    ...
    #[test]
    fn no_deepest_call_yields_succeeded() {
        let calls = map_with(contract_call_with_result(Err(
            TraceDataCallFailure::Recoverable {
                panic_data: vec![StarknetFelt::from_bytes_be_slice(b"ignored")],
            },
        )));

        // Without a deepest call id there is no failing call to report on.
        let result = get_execution_result(&calls, None, None, None).unwrap();

        assert!(matches!(result, ExecutionResult::Succeeded));
    }

    #[test]
    fn failed_result_marks_contract_call_as_failed() {
        let call = contract_call_with_result(Err(TraceDataCallFailure::Recoverable {
            panic_data: vec![StarknetFelt::from_bytes_be_slice(b"boom")],
        }));
