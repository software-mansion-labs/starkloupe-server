...
impl Tool {
    ...
    ///
    /// The version is written out whole, so a prerelease keeps its `-alpha.1`
    /// and gets a name of its own rather than one already spoken for. Only
    /// Sozo ever has one to keep; see `installs_prereleases`.
    ...
    /// Whether a prerelease of this tool can be installed at all.
    ///
    /// Sozo's can: the name is built from the tag, so `v1.2.0-alpha.1` installs
    /// as `sozo_v1.2.0-alpha.1`, which is exactly what a project pinning that
    /// tag in its Scarb.toml asks for. Dropping them would stop a project on a
    /// Dojo alpha from being verified at all.
    ///
    /// Scarb's cannot: its name is the Cairo version the binary reports, and
    /// `extract_cairo_version` reads it with `cairo: ([\d\.]+)`, which stops at
    /// the `-`. An rc would install under the name of the stable release it
    /// precedes and stand in for a version that has not shipped. Nothing is
    /// lost by skipping them, because a Cairo version reachable only through an
    /// rc cannot be asked for either: a manifest carries a three-number tuple.
    fn installs_prereleases(self) -> bool {
        match self {
            Tool::Scarb => false,
            Tool::Sozo => true,
        }
    ...
    fn is_supported(self, version: &Version) -> bool {
        match self {
            ...
            // Written out whole: a prerelease has to be judged as itself, or
            // the check would install `1.1.0-alpha.1` on the strength of what
            // `1.1.0` is worth and the verifier would then refuse it.
            Tool::Sozo => is_dojo_version_supported(&version.to_string()),
        }
    }
}
...
impl BucketPublisher {
    ...
    /// Put a binary in the bucket, replacing whatever is under that key.
    ///
    /// It uploads only what it has just installed, and only under the plain
    /// prefix, so what it can replace is another machine's copy of the same
    /// version - the same release asset either way.
    ///
    /// A build that has to survive a release check does not live here. It goes
    /// under `modified/`, which nothing in this module writes to and which the
    /// startup sync applies last, on top of this one
    /// (crates/server/src/binaries_manager_service.rs).
...
pub async fn check_for_updates(
    ...
    let candidates = installable_candidates(tool, releases);
...
///
/// For a tool that cannot install one, prereleases are dropped in both
/// spellings — the flag upstream sets, and a `-rc.N` in the tag — because
/// either alone misses the releases marked only the other way. Which tool that
/// is, and why, is `installs_prereleases`.
fn installable_candidates(tool: Tool, releases: Vec<Release>) -> Vec<(Version, Release)> {
    let keeps_prereleases = tool.installs_prereleases();
    let mut candidates: Vec<(Version, Release)> = releases
        .into_iter()
        .filter(|release| keeps_prereleases || !release.prerelease)
        .filter_map(|release| {
            let version = Version::parse(&parse_version_from_tag(&release.tag_name)).ok()?;
            (keeps_prereleases || version.pre.is_empty()).then_some((version, release))
        })
        .collect();
...
mod tests {
    ...
    fn considers_releases_newest_first() {
        let candidates = installable_candidates(
            Tool::Scarb,
            vec![
    ...
    #[test]
    fn drops_scarb_prereleases_however_they_are_marked() {
        let candidates = installable_candidates(
            Tool::Scarb,
            vec![
    ...
    #[test]
    fn keeps_sozo_prereleases_because_a_project_can_pin_one() {
        // A Dojo alpha gets a name of its own, so a Scarb.toml pinning that tag
        // finds the binary. Dropping these would leave such a project unable to
        // be verified at all.
        let candidates = installable_candidates(
            Tool::Sozo,
            vec![
        ...
        let order: Vec<&str> = candidates
            ...
            .map(|(_, release)| release.tag_name.as_str())
            .collect();
        assert_eq!(order, ["v1.2.0", "v1.2.0-alpha.1"]);
        assert_eq!(
            Tool::Sozo.object_name(&candidates[1].0),
            "sozo_v1.2.0-alpha.1"
        );
    }
