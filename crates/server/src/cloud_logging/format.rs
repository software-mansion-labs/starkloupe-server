...
//!
//! Replaces `sentry_tracing::layer()`. That layer turned `tracing::error!` into
//! a Sentry issue and everything below it into a breadcrumb; this one turns
//! every event into a `LogEntry` and marks the error-level ones so Cloud Error
//! Reporting picks them up out of the log stream.
//!
...
//!
//! On grouping: Sentry fingerprinted on the stack trace, so an interpolated
//! error message cost nothing. Error Reporting has no Rust stack trace to work
//! with and falls back to the message text plus `context.reportLocation`, so
//! `error!("failed for {address}")` fans out into one group per address. The
//! fix is to keep the varying part in a structured field -
//! `error!(%address, "failed")` - which groups on the constant text and keeps
//! the address queryable. Existing call sites still interpolate; they group by
//! `reportLocation` well enough to be useful and can be converted as they are
//! touched.
...
use super::{request, service_context};
...
pub fn layer<S>() -> tracing_subscriber::fmt::Layer<S, JsonFields, CloudLoggingFormatter>
where
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    tracing_subscriber::fmt::layer()
        .json()
        .event_format(CloudLoggingFormatter)
}
...
impl<S> FormatEvent<S, JsonFields> for CloudLoggingFormatter
where
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    fn format_event(
        ...
        if let Some(leaf) = context.lookup_current() {
        ...
        // Request scope, when there is one. Absent for the startup path and the
        // background schedulers, which is correct - they have no request.
        request::with_current(|scope| {
            entry.insert("httpRequest".into(), scope.log_entry_http_request());
            if let Some(trace) = scope.trace_field() {
                entry.insert("logging.googleapis.com/trace".into(), trace.into());
            }
            if let Some(span_id) = scope.span_id() {
                entry.insert("logging.googleapis.com/spanId".into(), span_id.into());
            }
            if scope.trace_sampled() {
                entry.insert("logging.googleapis.com/trace_sampled".into(), true.into());
            }
        });
...
pub(super) fn decorate_as_reported_error(
    ...
    // Error Reporting's httpRequest is a different, smaller shape than the
    // LogEntry one already on the entry, so it gets built separately rather
    // than reused.
    request::with_current(|scope| {
        error_context.insert("httpRequest".into(), scope.error_context_http_request());
    });
...
mod tests {
    ...
    fn info_event_is_a_plain_log_entry() {
        let entry = capture(|| tracing::info!(chain_id = "SN_MAIN", "simulating"));

        assert_eq!(entry["severity"], "INFO");
        assert_eq!(entry["message"], "simulating");
        assert_eq!(entry["chain_id"], "SN_MAIN");
    ...
    fn error_event_carries_the_reported_error_marker() {
        let entry = capture(|| tracing::error!("Failed to convert contract address"));

        assert_eq!(entry["severity"], "ERROR");
        assert_eq!(entry["@type"], REPORTED_ERROR_EVENT_TYPE);
        assert_eq!(entry["message"], "Failed to convert contract address");
    ...
    fn warnings_are_not_reported_as_errors() {
        let entry = capture(|| tracing::warn!("Failed to initialize Voyager client"));
    ...
    fn span_fields_are_merged_into_the_entry() {
        let entry = capture(|| {
            let span = tracing::info_span!("verify", verification_id = 7);
            let _entered = span.enter();
            tracing::info!("started");
        });

        assert_eq!(entry["verification_id"], 7);
