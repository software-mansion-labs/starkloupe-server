...
impl SimulationCache {
    pub fn new(max_capacity: u64, ttl_minutes: u64) -> Self {
        ...
        info!("Initialized cache with capacity: {}, TTL: {}min", max_capacity, ttl_minutes);
        ...
        *ops_count += 1;
        ...
        if current_entries < *last_count && (*last_count - current_entries) > 0 {
            ...
            if cleaned > 1 || current_entries == 0 {
                info!("[CACHE CLEAN] {} entries removed (LRU eviction or TTL expiration)", cleaned);
            }
        }
                    ...
                    *hits += 1;
                    ...
                    match &cached.result {
                        ...
                        CachedResult::Debug(_) => {
                            warn!("[CACHE ERROR] tried to get simulation but found debug {}", key.display_id());
                            None
                        }
                    }
                }
                    ...
                    *hits += 1;
                    ...
                    match &cached.result {
                        ...
                        CachedResult::Simulation(_) => {
                            warn!("[CACHE ERROR] tried to get debug but found simulation {}", key.display_id());
                            None
                        }
                    }
                }
